...
class AddPasswordWidget(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        self.name_entry = ctk.CTkEntry(form_frame, placeholder_text="Aplicación", width=250)
        ...
        self.user_entry = ctk.CTkEntry(form_frame, placeholder_text="Usuario", width=250)
    ...
    def add_pass(self):
        ...
        if user_input_validation:
            error, status, data  = self.parent_app.add_pass(["", self.name_entry.get().capitalize(), self.user_entry.get(), self.password_entry.get(), self.email_entry.get(), self.app_id_entry.get(), self.url_entry.get()])
            if error:
