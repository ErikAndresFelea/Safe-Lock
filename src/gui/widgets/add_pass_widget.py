...
class AddPasswordWidget(customtkinter.CTkFrame):
    def __init__(self, master, app):
        ...
        self.top_frame.grid_rowconfigure((0, 1, 2, 3, 4, 5), weight=1)
        ...
        self.confirm_password_label = customtkinter.CTkLabel(self.top_frame, text="Confirmar contraseña")
        self.confirm_password_label.grid(row=2, column=0, padx=20, pady=5, sticky="w")
        self.repeat_password_entry = customtkinter.CTkEntry(self.top_frame, placeholder_text="******", width=250)
        self.repeat_password_entry.grid(row=2, column=1, padx=20, pady=5)
    ...
    def add_pass(self):
        if self.password_entry.get() == self.repeat_password_entry.get():
            password_data = ["", self.name_entry.get().capitalize(), self.password_entry.get(), self.repeat_password_entry.get(), self.email_entry.get(), self.app_id_entry.get(), self.url_entry.get()]
