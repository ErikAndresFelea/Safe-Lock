...
class EditPasswordWidget(customtkinter.CTkFrame):
    def __init__(self, master, app, data: list[str]):
        ...
        self.edit_button = customtkinter.CTkButton(self.bottom_frame, text="Editar", command=self.update_pass, width=75)
    ...
    def update_pass(self):
        password_data = [self.password_data[0], self.name_entry.get().capitalize(), self.password_entry.get(), self.email_entry.get(), self.app_id_entry.get(), self.url_entry.get()]
        self.parent_app.update_pass(password_data)