...
class DeleteUserWidget(ctk.CTkFrame):
    def __init__(self, master: ctk.CTk, app: App) -> None:
        ...
        self.__delete_user_entry = ctk.CTkEntry(top_frame, placeholder_text="Escribe 'Borrar' para proceder", width=250)
        self.__delete_user_entry.grid(row=1, column=0, padx=5, pady=(0, 5))
        ...
        delete_button = ctk.CTkButton(bottom_frame, text="Borrar", border_color="white", border_width=1, command=self.__update_pass, width=75)
        delete_button.grid(row=0, column=1, padx=5, pady=(30, 5), sticky="w")
    ...
    def __update_pass(self) -> None:
        ...
        if user_input_validation:
            operation  = self.__parent_app.controller.delete_account()
            if not operation:
                self.__error_label.configure(text="Error al borrar la cuenta")
            else:
    ...
    def __check_user_input(self) -> bool:
        delete = self.__delete_user_entry.get().lower() == 'borrar'

        # Update UI with msg & color feedback
        if not delete:
            self.__error_label.configure(text="Escribe 'borrar' para eliminar la cuenta") 
            self.__delete_user_entry.configure(border_color="darkred")
        return delete
    ...
    def __reset_ui(self) -> None:
        ...
        self.__delete_user_entry.configure(border_color="gray50")
